...
public interface MessageRepository extends CrudRepository<Message, Object> {
    ...
    @Query(value = "SELECT message.* FROM message WHERE (author_id = :authorId OR receive_id = :authorId) AND id IN ( SELECT MAX(id) FROM message WHERE author_id <> :authorId OR message.receive_id <> :authorId GROUP BY CASE WHEN author_id = :authorId THEN receive_id ELSE author_id END ) ORDER BY message.id DESC", nativeQuery = true)
